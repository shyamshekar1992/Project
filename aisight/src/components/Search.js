...
const Search = () => {
  ...
  const handleSubmit =(e) =>{
    e.preventDefault();
      axios.get(constants.backend_url + 'api/v1/movie/getMovies')
          .then(response => {
  ...
  const updateList =(e) =>{
    ...
    axios.get('http://www.omdbapi.com/?apikey=8b006a97&s=space')
        .then(response => {
          response.data.Search.forEach(movie=>
              axios.get('http://www.omdbapi.com/?apikey=8b006a97&i='+movie.imdbID)
                  .then(response => {
                    if(response.data.Year>="2001"){
                      axios.post(constants.backend_url + 'api/v1/movie/add/', response.data)
                          .then(response=>{
    ...
    return (
      <div
        ...
        style={{ marginTop: "8%" }}
      >
        <button className="btn btn-primary" onClick={updateList}>Update Database</button>
        <br></br>
          ...
          >
              {productsList != '' ?
                  <main className="container">
                      <div className="h1 text-center" id="pageHeaderTitle">Search Results</div>
